...
public class SendingDomainTest
{
    ...
    public async Task ListSendingDomains_returns_expected_result(SparkPostEnvironment env)
    {
        var response = await SendingDomainExtensions.ListSendingDomains()(env).IfFailThrow();

        response.Should().BeRight();
    }
    
    private class SendingDomainAutoDataAttribute()
